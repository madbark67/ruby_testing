...
describe Dog do
  ...
  subject(:dog) { described_class.new('Bark', 'Pitbull', 'Brown') }
  # Write a test using the second shared_example to test that dog responds to
  # talk ('WOOF!').
  context 'When dog shares a method name with another class' do
    include_examples 'shared method name'
  end

  # remove the 'x' before running this test
  it 'is not barking' do
    expect(dog).not_to be_barking
  end

  # remove the 'x' before running this test
  it 'is sleeping' do
    expect(dog).to be_sleeping
  end
end
