...
describe BinaryGame do
  ...
  describe '#player_input' do
    ...
    context 'when user inputs an incorrect value once, then a valid input' do
      before do
        allow(game_input).to receive(:gets).and_return('s', '5')
      end

      it 'completes loop and displays error message once' do
        minimum = game_input.instance_variable_get(:@minimum)
        maximum = game_input.instance_variable_get(:@maximum)
        error_message = "Input error! Please enter a number between #{minimum} or #{maximum}."
        expect(game_input).to receive(:puts).with(error_message).once
        game_input.player_input(minimum, maximum)
        expect(game_input.player_input(minimum, maximum)).to eq(5)
      end
    end

    context 'when user inputs two incorrect values, then a valid input' do
      before do
        allow(game_input).to receive(:gets).and_return('s', 't', '5')
      end
      ...
      it 'completes loop and displays error message twice' do
        ...
        game_input.player_input(min, max)
        game_input.player_input(min, max)
        expect(game_input.player_input(min, max)).to eq(5)
      end
    end
  end
  ...
  describe '#verify_input' do
    ...
    context 'when given a valid input as argument' do
      it 'returns valid input' do
        min = game_input.instance_variable_get(:@minimum)
        max = game_input.instance_variable_get(:@maximum)
        expect(game_input.verify_input(min, max, 5)).to eq(5)
      end
    end
    ...
    context 'when given invalid input as argument' do
      it 'returns nil' do
        min = game_input.instance_variable_get(:@minimum)
        max = game_input.instance_variable_get(:@maximum)
        expect(game_input.verify_input(min, max, 12)).to be_nil
      end
    end
  end
  ...
  describe '#display_turn_order' do
    ...
    it 'increases guess_count by one' do
      expect { game.display_turn_order(search) }.to change { game.instance_variable_get(:@guess_count) }.by(1)
    end

    # Method with Outgoing Command -> Test that a message is sent
    it 'sends make_guess' do
      expect(search).to receive(:make_guess).once
      game.display_turn_order(search)
    end
